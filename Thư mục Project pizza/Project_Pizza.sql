--Check dữ liệu trùng nhau/ Duplicated data
select * from pizza_sales;
with b as (select*, row_number() over (partition by order_id, pizza_name_id,order_time,order_date,pizza_size,pizza_category order by order_id) as rn from pizza_sales)
select * from b
where rn >1;
-- Không có dữ liệu trùng/ no duplacted data
-- Check dữ liệu null toàn bảng
...
begin
	...
	SUM(CASE WHEN ['+COLUMN_NAME+']is null then 1 else 0 end) as sonull_'+ COLUMN_NAME,
	','
	)
...
Exec Checknull 'pizza_sales';
-- INFORMATION_SCHEMA.COLUMNS một hệ thống schema chuẩn chứa các view metadata giúp bạn tra cứu thông tin về database.
-- Tham khảo thêm về EXEC(@SQL) và Exec sp_executesql @SQL;
-- Chú ý khoảng trắng giữa các câu lệnh select

--- KẾT QUẢ: KHÔNG CÓ DỮ LIỆU NULL /NONULL
...
order by sum(distinct([order_id])); 
...
-- Truy xuất dữ liệu số lượng bán được theo loại pizza
select rank() over (order by sum(p.quantity) DESC) as Rankluongbandc,sum(p.quantity) as Tongbandc,p.SoNglieu
from pizza_sales p
group by p.SoNglieu
-- Vậy có thể thấy rằng càng nhiều nguyên liệu trong pizza thì càng được yêu thích
-- pizza càng nhiều nguyên liệu thì giá cả càng cao không?
select AVG(p.unit_price) as giaTB, rank() over(order by AVG(p.unit_price) Desc) as rank,p.SoNglieu
from pizza_sales p
group by p.SoNglieu
-- Gía TB của pizza 6 nguyên liệu cao nhất nhưng vẫn được yêu thích nhất
-- các loại pizza 6 nguyên liệu gồm các thành phần gì
select p.pizza_name,p.pizza_ingredients,p.SoNglieu, sum(p.quantity) as Tongbandc,p.unit_price
from pizza_sales p
where p.SoNglieu = 6
group by p.pizza_name,p.pizza_ingredients,p.SoNglieu,p.unit_price
order by sum(p.quantity) DESC
-- The five cheese pizza được yêu thích nhất giá rẻ 18.5 nguyên liệu đơn giản
